...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
        ...
        if representation_size and not distilled:
            ...
            self.pre_logits = nn.Sequential(OrderedDict([                         # [768,768]转换
    ...
    def forward_features(self, x):
        ...
        x = self.blocks(x)  #通过attention堆叠模块（12个）      # [B, 197, 768]
        x = self.norm(x)  #layer_norm
        if self.dist_token is None:
            return self.pre_logits(x[:, 0])  #返回第一层特征，即为分类值      x[:, 0]: [B,768] cls      已经经过[768,768]
        else:
            return x[:, 0], x[:, 1]

    def forward(self, x):
        # 分类头
        x = self.forward_features(x) # 经过att操作，但是没有进行分类头的前传        # [B,768]
        '''
